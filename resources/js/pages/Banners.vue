<template>
  <div>
    ...
    <form class="mt-4" @submit.prevent="upload" method="post">
      <h2 class="text-base">Upload a Banner</h2>
      <div class="mt-4">
        ...
        <div>
          ...
          <button
            type="button"
            @click="openUploader"
            class="px-4 py-2 w-full max-w-sm rounded bg-mantis-500 hover:bg-mantis-600 text-white transition duration-150 ease-in"
          >
      ...
      <div class="mt-4 max-w-sm">
        ...
        <span v-if="urlError" class="text-red-500 pb-1"
          ><em>{{ urlError }}</em></span
        >
        <input
          ref="bannerUrlInput"
          ...
          required
          type="text"
          class="w-full px-4 py-2 border border-gray-300 rounded placeholder-gray-600"
          :class="urlError ? 'border-red-500' : ''"
          @blur="validateUrl"
          placeholder="Enter the URL asscoiated with the banner."
        />
      </div>

      <div class="mt-4">
        <button
          class="bg-mantis-500 hover:bg-mantis-600 text-white rounded px-4 py-2 transition duration-150 ease-in"
        >
          Upload
        </button>
      </div>
          ...
          <tr v-for="banner in banners" class="group hover:bg-gray-200">
            ...
            <td class="border-t border-b px-4 py-2">
              <button
                ...
                type="button"
                class="invisible group-hover:visible px-4 py-2 border rounded border-red-500 text-red-500 hover:bg-red-200 hover:text-red-800"
              >
...
export default {
  ...
  methods: {
    ...
    upload() {
      ...
      this.$http
        .post("/api/banners", formData, config)
        .then(response => {
          flash({
            type: "success",
